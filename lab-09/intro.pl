...
myReverseAcc([], L, L).
myReverseAcc([X|Rest1], Acc, RevList):- myReverseAcc(Rest1, [X|Acc], RevList).
...
% == Primul caz ==
setIntersection([X|Rest1], L2, [X|Intersectie]):- member(X, L2), setIntersection(Rest1, L2, Intersectie).
% Intersectie -- Intersectia dintre Rest1 si L2

% == Al 2lea caz ==
setIntersection([X|Rest1], L2, Intersectie):- \+member(X, L2), setIntersection(Rest1, L2, Intersectie).
...
% == Primul caz ==
setDiff([X|Rest1], L2, [X|Diferenta]):- \+member(X, L2), setDiff(Rest1, L2, Diferenta).
% Diferenta -- Diferenta dintre Rest1 si L2

% == Al 2lea caz ==
setDiff([X|Rest1], L2, Diferenta):- member(X, L2), setDiff(Rest1, L2, Diferenta).
...
setUnion(L1, L2, L):- setIntersection(L1, L2, Intersectie), setDiff(L2, Intersectie, Diferenta), myConcat(L1, Diferenta, L).
% Intersectie -- L1 intersectat cu L2
% Diferenta -- L2 - Intersectie
% L -- L1 ++ Diferenta
...
descendantOf(X, Y):- arc(Urmas, X), descendantOf(Urmas, Y).
...
sameTree(N1, N2):- descendantOf(N1, N2).
sameTree(N1, N2):- descendantOf(N2, N1).
sameTree(N1, N2):- isRoot(Radacina), descendantOf(N1, Radacina), descendantOf(N2, Radacina).
