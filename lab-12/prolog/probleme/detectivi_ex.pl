...
% Gaseste toti suspectii care conduc bmw,
% creeaza 'ListaArme'
...
arme_marca(Marca, ListaArme) :- setof(Arma, X^suspect(X:Marca:Arma), ListaArme).
...
marci_arma_unica(ListaMarci) :- 
    findall(Masina, (conduce(_, Masina), arme_marca(Masina, [_])), ListaMarci).
