...
permutari4([Elem1, Elem2, Elem3, Elem4], Permutari) :-
        length(Permutari, 4), % Permutari are 4 elemete
        member(Elem1, Permutari), member(Elem2, Permutari),  % Toate elementele din lista
        member(Elem3, Permutari), member(Elem4, Permutari).  % se regasesc in output
...
alternare(_, _, 0, []) :- true.
alternare(A, B, Len, [A | Res]) :-
        X is Len - 1, Len > 0, % Conditii, Len scade cu 1 la fiecare pas
        alternare(B, A, X, Res). % Pas alternare
...
sublista(_,_) :- false.
sublista([],[]) :- true.
sublista([X | Y], [X | Rest]) :- sublista(Y, Rest).
sublista([_ | Y], Rest) :- sublista(Y, Rest).
...
% Lista s-a terminat
extract([], _, _, []) :- true.
% S-a ajuns la finalul zonei de extras
extract(_, _, 0, []) :- true.
% Nu s-a ajuns inca la inceputul zonei de extras
extract([_ | Y], Start, End, Range) :-
    S is Start - 1,
    E is End - 1,
    extract(Y, S, E, Range).
% Extragere
extract([X | Y], 0, End, [X | Range]) :-
    S is 0,
    E is End - 1,
    extract(Y, S, E, Range).
