...
(define (deep-rev L)
  (cond
    ((null? L) '()) ; caz de baza
    ((list? (car L))
     (append (deep-rev (cdr L))
             (list (deep-rev (car L))))) ; inverseaza si elementele listelor din interior (caz 3)
...
(define (palindrome? L)
  (equal? L (rev L))) ; M-am folosit de rev
...
(define (palindrome-list? L)
  (equal? (rev L) (deep-rev L))) ; Daca toate listele dil L sunt palindroame, atunci si daca facem
                                 ; deep-rev ar trebui sa arate la fel
