...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segment (get-line-segment))
         (A (get-start-point segment))
         (B (get-end-point segment))
         (x1 (car A))
         (y1 (cdr A))
         (x2 (car B))
         (y2 (cdr B))
         )
    (sqrt (+ (sqr (- x1 x2)) (sqr (- y1 y2))));AB = radical((x1 - x2)^2 + (y1 - y2)^2)
  )
)
...
(define (num-concat x y)
  (let ((+ (λ (x y) (string->number (string-append (number->string x)
                                                   (number->string y)
                                    )
                    )
            )
...
(define (compute-perimeter points)
  (let ((A (car points)))
    (let iter ((lista_puncte points) (perimetru 0))
      (if (null? (cdr lista_puncte))
        (+ perimetru (distance (car lista_puncte) A))
      (iter (cdr lista_puncte) (+ perimetru (distance (car lista_puncte) (cadr lista_puncte))))
      )
    )
  )
)
