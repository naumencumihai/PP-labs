...
safeTaran([]).
safeTaran([_]).
safeTaran(List) :- sort(List, [lup, varza]).
...
parseState(state(Mal, NME, NCE, NMV, NCV), Mal, NME, NCE, NMV, NCV).
...
initial_state(misionari, state(est, NME, NCE, NMV, NCV)) :-
	NME is 3,
	NCE is 3,
	NMV is 0,
	NCV is 0.
...
final_state(misionari, state(vest, NME, NCE, NMV, NCV)) :-
	NME is 0,
	NCE is 0,
	NMV is 3,
	NCV is 3.
...
next_state(misionari, state(est, NME1, NCE1, NMV1, NCV1), state(vest, NME2, NCE2, NMV2, NCV2)) :-
	boat(NM, NC), NM =< NME1, NC =< NCE1,
	NME2 is NME1 - NM, NCE2 is NCE1 - NC,
	NMV2 is NMV1 + NM, NCV2 is NCV1 + NC,
	safeMisionari(NME2, NCE2),
	safeMisionari(NMV2, NCV2).

next_state(misionari, state(vest, NME1, NCE1, NMV1, NCV1), state(est, NME2, NCE2, NMV2, NCV2)) :-
	boat(NM, NC), NM =< NMV1, NC =< NCV1,
	NME2 is NME1 + NM, NCE2 is NCE1 + NC,
	NMV2 is NMV1 - NM, NCV2 is NCV1 - NC,
	safeMisionari(NME2, NCE2),
	safeMisionari(NMV2, NCV2).
...
% Daca X este ultimul nod
% (acesta se regaseste si in solutie)
bfs([(X, P) | _], C, [(X, P) | C]) :-
	final_node(X),
	!.

% Daca nodul X este deja vizitat, il sarim
% (daca face parte din lista "Closed" (C))
bfs([(X, _) | Rest], C, S) :-
	member((X, _), C),
	!,
	bfs(Rest, C, S).

% Adauga succesorii(Succ) nodului X la lista "Frontier" (F) si
% continua cautarea bfs, nodul curent (X) fiind adaugat in
% lista nodurilor vizitate (C)
bfs([(X, P) | Rest], C, S) :-
	getNeighb(X, [], Succ),
	append(Rest, Succ, F),
	bfs(F, [(X, P) | C], S).
...
% Incepe constructia caii, plecand de la nodul final (h)
extract_path(D, S) :-
	final_node(N),
	extract_path(D, [N], S).

% Adauga nodul radacina (a) la solutie (S)
extract_path(_, [H | T], [H | T]) :-
	initial_node(H),
	!.


extract_path(D, [H | T], S) :-
	member((H, P), D),
	extract_path(D, [P, H | T], S).
