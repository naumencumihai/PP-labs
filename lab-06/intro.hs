...
numToBase2 0 _ = []
numToBase2 n b = (numToBase2 (n `quot` b) b) ++ [(n `mod` b)]
...
setIntersection a b = [x | x <- a, x `elem` b] -- x din a si x element al lui b

setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [x | x <- a, x `notElem` b] -- x din a si x NU e element al lui b

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(x, y) | x <- a, y <- b] -- pereche (x, y), unde x din a si y din b

setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = a ++ setDiff b (setIntersection a b) -- tot a ++ restul din b care nu sunt si in a
