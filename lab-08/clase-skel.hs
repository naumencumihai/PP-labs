...
instance Show TicTacToe
    where 
        show (Board board) = first ++ "\n" ++ middle ++ "\n" ++ last
            where
                first = concatMap show $ take 3 board -- [x Zero Space] -> ["X" "0" " "] -> "X0 "
...
class (Ord a) => PQueue pq a where
    ...
    fromList = foldr insert empty -- poit free programming
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert node (LPQ q) = LPQ (insert node q)
        where
            insert node [] = [node]
            insert node q@(x:xs)
